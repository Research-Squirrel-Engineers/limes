...
file_in = dir_path.replace("\\py","\\csv") + "\\" + "AntonineItinerary.csv"
file_out = dir_path.replace("\\py","\\geojson") + "\\" + "AntonineItinerary.geojson"
...
data = pd.read_csv(
    ...
    usecols=['group','name','roman_name','city','county','grid','long','lat'], # only load the  columns specified
...
for index, row in data.iterrows():
    properties = {}
    properties['group'] = int(row['group'])
    properties['name'] = row['name']
    properties['roman_name'] = row['roman_name']
    properties['city'] = row['city']
    properties['county'] = row['county']
    properties['grid'] = row['grid']
    #print(properties)
